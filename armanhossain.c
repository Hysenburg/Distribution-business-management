...
void searchItem() {
    ...
    for (int i = 0; i < ICount; i++) {
        if (strcmp(Item[i].itemname, itemName) == 0) {
            printf("%s is in stock. Available quantity: %d\n", itemName, Item[i].stock);
            found = 1;
            break;
        }
    }
...
void upgradeProduct() {
    display();
    if (ICount == 0) {
        printf("No products in stock to upgrade.\n");
        return;
    }

    printf("Enter the number of the product you want to upgrade: ");
    int upgradeIndex;
    scanf("%d", &upgradeIndex);

    if (upgradeIndex < 1 || upgradeIndex > ICount) {
        printf("Invalid product number. Please enter a valid product number.\n");
        return;
    }

    printf("Enter the new price of the product: ");
    scanf("%f", &Item[upgradeIndex - 1].itemprice);

    printf("Enter the new stock of the product: ");
    scanf("%d", &Item[upgradeIndex - 1].stock);
...
int main() {
    ...
    while (1) {
        ...
        printf("5. Search Item in a stock\n");
        printf("6. Upgrade product\n");
        printf("7. Exit\n");
